...
const getTimeStamp = (time)=>{
    const INTERVAL = 30*60*1000 // 30mins in ms;
    //lets convet the given timestamp in ms 
    const date = new Date(time)
     time = date.getTime();

    // console.log("Before flooring", time);
    
    const intervalPoint = Math.floor(time/INTERVAL)*INTERVAL;
    // console.log("After: ")

    return new Date(intervalPoint).toISOString();
}

const binnedTable = events.reduce((prevEvent, currentEvent) => {
    const bin = getTimeStamp(currentEvent.timestamp);

    if(!prevEvent[bin]){
        prevEvent[bin] = {total: 0};
    }
    prevEvent[bin].total += 1;
    return prevEvent
},{})
...
// console.log(getTimeStamp("2025-10-22T10:05:00Z"))